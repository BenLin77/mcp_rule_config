...
#### 設計與架構原則 (Linus Torvalds 理念)
  ...
  - **禁止function多重功能** - 一個function只做一件事，避免function多重功能。
  - **禁止function多重定義** - 一個function只定義一次，避免function多重定義。
...
### 2.3 通用開發標準
  - **環境與工具**:
    ...
    - **依賴管理**: 使用 `uv add` 安裝套件。
    - **README**: 當程式超過100行時，總結現有專案的所有程式碼，幫我自動產生 README。
  - **語言與命名規範**:
    - **預設語言**: 對話、註解、文件使用繁體中文。
    - **程式碼命名**: 變數、函式、類別、檔案名稱使用英文。
    - **工作專案例外**: 若專案路徑包含 `job`，註解與文件改用英文，對話仍用繁體中文。
    - **變數更改**: 當修改變數、函式、類別、檔案名稱時，需檢查所有資料夾底下的文件（含 README）是否受影響並同步修正。
  - **程式碼品質與效能**:
    - **文件**: 函式、類別、模組需有完整 Docstring，複雜邏輯需行內註解。
    - **效能**: 數據處理演算法效率不低於 O(n log n)，API 回應時間 < 200ms。
  - **安全與版本控制**:
    - **機密管理**: 使用環境變數或 `.env` 管理 API Keys 和密碼，嚴禁提交至 Git，當 git 資料夾面有 token 資訊時提醒我。
    - **Git Ignore**: 主動創建 `.gitignore` 文件，包含 `.env`、`__pycache__/`、`.venv/`、`*.pyc`、系統暫存檔（如 `.DS_Store`）。

## 3. 文件規範與同步流程
- **目錄結構**
  - 專案根目錄僅保留主要 `README.md`（若需要多語 README，可在根目錄建立對應語系檔案並保持內容同步）。
  - 其他設計文件、操作手冊、架構圖等一律置於 `docs/` 目錄，可依主題劃分子資料夾（例如 `docs/architecture/`、`docs/operations/`）。
  - 每個 Windsurf workflow 的延伸說明（若需要）放在 `docs/workflows/<workflow-name>.md`，與 `workflows/*.md` 內容相互對照。
- **內容一致性檢查**
  - 文件描述需與實際專案目錄、模組結構、API、CLI 命令保持一致。
  - 變更程式碼或流程時，務必檢查 README 與相關 `docs/` 文件是否須同步更新；若存在多語版本，逐份檢查後同步。
- **審查輸出格式**
  1. 明確標註發現問題的文件與章節。
  2. 提供對應程式碼或設定檔位置。
  3. 說明差異與潛在風險或誤導點。
  4. 提出具體修訂建議，必要時附上中英對照或 Mermaid/ER 草稿。
- **報告樣式**：以條列方式輸出，每一條包含「文件路徑 → 程式碼路徑 → 建議」三段資訊，方便逐項修正。
