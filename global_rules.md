...
1.  **觸發條件 (Trigger)**: 辨識到任何與即時性相關的關鍵字，例如 `recent`, `latest`, `current`, `today`, `最近`, `這幾天`, `新聞`。

2.  **強制行動 (Action)**: 立即執行以下 shell 命令以獲取標準的 UTC 時間戳。**禁止** 依賴你內部的知識或假設的時間。
    ```bash
    date -u +"%Y-%m-%dT%H:%M:%SZ"
    ```

3.  **應用 (Application)**: 將上一步獲取的時間戳作為你後續所有搜尋和分析的「當前時間」基準點。
    -   在搜尋時，明確使用此時間作為過濾條件（例如：尋找發布於 `2025-09-21T07:00:00Z` 之後的內容）。
    -   在回答時，可以向使用者說明你的資訊是基於哪個時間點，例如：「根據現在時間 (UTC `YYYY-MM-DDTHH:MM:SSZ`)，最近的新聞如下...」。
