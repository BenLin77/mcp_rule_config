...
### 3.4 使用 Gemini CLI 進行程式碼庫分析與輔助撰寫
此標準旨在建立一個統一的開發流程：凡是涉及程式碼分析、比較或文件撰寫等任務，一律優先使用 **Gemini CLI**。此工具能有效處理不同規模的專案，不僅能應對超大型程式碼庫，也能高效處理單一文件或小型目錄。透過 `gemini -p ` (pipeline) 模式，可以將程式碼內容直接作為上下文傳遞給模型，從而獲得更精準與深入的回答，在`gemini -p ` 中明文禁止刪除或修改檔案。

**核心功能與應用場景**

- **程式碼分析**：理解專案架構、模組依賴、資料流或程式碼品質。
- **輔助撰寫**：快速生成專案 README、API 文件或內部技術筆記。
- **程式碼比較與重構**：比較不同版本、找出重複程式碼、並給出重構建議。

**基本用法**

1.  **分析整個目錄**：
    ```bash
    gemini -p . "說明此專案的核心模組拓樸與資料流"
    ```
    此命令會將整個 repo 的內容整合進模型上下文，回答你的提問。

2.  **指定特定檔案或子目錄**：
    ```bash
    gemini -p src/backend/utils.py "解釋此檔案中的所有函式用途"
    ```
    針對單一檔案進行精確分析，適用於快速理解特定模組。

3.  **生成 README 或文件**：
    ```bash
    gemini -p . "請根據此專案內容，撰寫一份 README.md，包含安裝、使用與貢獻說明"
    ```
    將整個專案作為上下文，直接產出高質量的專案文件。